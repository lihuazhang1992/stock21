...
class PriceBreakoutMonitor:
    def __init__(self, root):
        self.root = root
        self.root.title("价格突破/反弹监测")
        self.root.geometry("600x400")
        
        # 模拟价格数据（实际使用时替换为真实行情数据）
        self.price_history = []  # 存储历史价格（模拟K线价格）
        self.support_period = 10  # 计算支撑位的周期（最近10根K线）
    ...
    def _init_ui(self):
        # 价格显示标签
        self.price_label = ttk.Label(self.root, text="当前价格：--", font=("Arial", 16))
        self.price_label.pack(pady=20)
        
        # 支撑位显示标签
        self.support_label = ttk.Label(self.root, text="关键支撑位：--", font=("Arial", 12))
        self.support_label.pack(pady=10)
        
        # 突破状态显示标签（核心新增）
        self.status_label = tk.Label(self.root, text="走势状态：--", font=("Arial", 14, "bold"))
    ...
    def _calculate_support_level(self):
        """计算关键支撑位（最近N根K线最低价）"""
        if len(self.price_history) < self.support_period:
            return min(self.price_history) if self.price_history else 0
        return min(self.price_history[-self.support_period:])

    def _judge_breakout_status(self):
        """判断价格状态：向下突破中/突破后反弹/无突破"""
        if len(self.price_history) < 2:
            return "数据不足", "#888888"  # 灰色
        ...
        if current_price < support_level:
            ...
            if current_price < prev_price:
                # 跌破后继续下跌 → 向下突破中
                return "向下突破中", "#FF4444"  # 红色
            else:
                # 跌破后开始上涨 → 突破后反弹
                return "突破后反弹", "#00C851"  # 绿色
        else:
            # 未跌破支撑位 → 无突破
            return "无向下突破", "#1E90FF"  # 蓝色

    def update_price(self):
        """模拟实时更新价格并刷新界面"""
        # 模拟生成价格（实际使用时替换为真实接口获取）
        base_price = 100.0 if not self.price_history else self.price_history[-1]
        new_price = base_price + random.uniform(-0.5, 0.5)
        self.price_history.append(round(new_price, 2))
        ...
        # 判断突破状态
        status_text, status_color = self._judge_breakout_status()
        
        # 更新界面显示
        self.price_label.config(text=f"当前价格：{new_price:.2f}")
        self.support_label.config(text=f"关键支撑位（近10期）：{support_level:.2f}")
        self.status_label.config(text=f"走势状态：{status_text}", fg=status_color)
