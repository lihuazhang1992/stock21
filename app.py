...
# --- 1. 基础配置与数据库连接 ---
st.set_page_config(page_title="股票管理系统 v23.0", layout="wide")
...
c.execute('''
    CREATE TABLE IF NOT EXISTS cycles (
        ...
        end_date TEXT,
        change_percent REAL,
        direction TEXT  -- 'up' 或 'down'
    )
''')
c.execute('''
    CREATE TABLE IF NOT EXISTS stock_peak (
        code TEXT PRIMARY KEY,
        peak_value REAL,
        peak_date TEXT
    )
...
# --- 2. 侧边栏导航 ---
menu = ["📊 实时持仓", "💰 盈利账单", "🎯 价格目标管理", "📝 交易录入", 
        "🔔 买卖信号", "📜 历史明细", "📓 复盘日记", "📋 个股档案"]   # 新增模块
...
# ========== 新增模块：📋 个股档案 ==========
    ...
    st.header("📋 个股档案 - 深度持仓分析")

    # 选择股票
    stock_list = get_dynamic_stock_list()
    selected_stock = st.selectbox("选择股票", [""] + stock_list, key="archive_stock")

    if selected_stock:
        # 获取基本数据
        trades_df = pd.read_sql(f"SELECT * FROM trades WHERE code = '{selected_stock}' ORDER BY date, id", conn)
        price_info = c.execute("SELECT current_price, manual_cost FROM prices WHERE code = ?", (selected_stock,)).fetchone()
        current_price = price_info[0] if price_info and price_info[0] else 0.0
        manual_cost = price_info[1] if price_info and price_info[1] else 0.0

        # 计算净持仓、市值、浮动盈亏
        if not trades_df.empty:
            net_buy = trades_df[trades_df['action'] == '买入']['quantity'].sum()
            net_sell = trades_df[trades_df['action'] == '卖出']['quantity'].sum()
            net_q = net_buy - net_sell
        else:
            net_q = 0

        market_value = net_q * current_price if net_q > 0 else 0.0
        if net_q != 0 and manual_cost > 0:
            float_profit = (current_price - manual_cost) * net_q if net_q > 0 else (manual_cost - current_price) * abs(net_q)
            float_profit_pct = (float_profit / (manual_cost * abs(net_q))) * 100 if manual_cost * abs(net_q) != 0 else 0.0
        else:
            float_profit = 0.0
            float_profit_pct = 0.0
                ...
                if action == '买入':
                    ...
                    if sell_positions:
                        for sp in sorted(sell_positions, key=lambda x: -x['price']):
                            ...
                            cover_qty = min(sp['qty'], remaining)
                            gain = ((sp['price'] - price) / sp['price'] * 100) if sp['price'] > 0 else 0.0
                            realized_profit += gain * sp['price'] * cover_qty / 100  # 将百分比转换为金额
                            sp['qty'] -= cover_qty
                            remaining -= cover_qty
                        sell_positions = [sp for sp in sell_positions if sp['qty'] > 0]
                    if remaining > 0:
                        buy_positions.append({'price': price, 'qty': remaining})
                else:  # 卖出
                    if buy_positions:
                        for bp in sorted(buy_positions, key=lambda x: x['price']):
                            if remaining <= 0:
                            ...
                            close_qty = min(bp['qty'], remaining)
                            gain = ((price - bp['price']) / bp['price'] * 100) if bp['price'] > 0 else 0.0
                            realized_profit += gain * bp['price'] * close_qty / 100
                            bp['qty'] -= close_qty
                            remaining -= close_qty
                        buy_positions = [bp for bp in buy_positions if bp['qty'] > 0]
                    if remaining > 0:
                        sell_positions.append({'price': price, 'qty': remaining})

        # 读取历史最高持仓金额
        peak_row = c.execute("SELECT peak_value, peak_date FROM stock_peak WHERE code = ?", (selected_stock,)).fetchone()
        peak_value = peak_row[0] if peak_row else 0.0
        peak_date = peak_row[1] if peak_row else ""

        # 读取交易逻辑
        logic_row = c.execute("SELECT logic FROM stock_logic WHERE code = ?", (selected_stock,)).fetchone()
        logic_text = logic_row[0] if logic_row else ""

        # ========== 显示区域 ==========
        st.subheader(f"📌 {selected_stock} 持仓概览")

        # 卡片式指标
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("持仓数量", f"{net_q}")
        col2.metric("成本价", f"{manual_cost:.3f}" if manual_cost else "未设置")
        col3.metric("现价", f"{current_price:.3f}" if current_price else "未设置")
        col4.metric("持仓市值", f"{market_value:,.2f}")

        col5, col6, col7, col8 = st.columns(4)
        col5.metric("浮动盈亏", f"{float_profit:,.2f}", delta=f"{float_profit_pct:.2f}%")
        col6.metric("已实现利润", f"{realized_profit:,.2f}")
        col7.metric("历史最高持仓金额", f"{peak_value:,.2f}" if peak_value else "未记录")
        col8.metric("最高日期", peak_date if peak_date else "-")

        # 历史最高持仓金额编辑
        with st.expander("✏️ 编辑历史最高持仓金额", expanded=False):
            col_a, col_b = st.columns(2)
            new_peak = col_a.number_input("最高金额", value=peak_value, step=1000.0, format="%.2f")
            new_peak_date = col_b.date_input("达到日期", value=datetime.strptime(peak_date, '%Y-%m-%d').date() if peak_date else datetime.now())
            if st.button("保存历史最高"):
                c.execute("INSERT OR REPLACE INTO stock_peak (code, peak_value, peak_date) VALUES (?, ?, ?)",
                          (selected_stock, new_peak, new_peak_date.strftime('%Y-%m-%d')))
                conn.commit()
                thread = threading.Thread(target=sync_db_to_github, daemon=True)
                thread.start()
                st.success("已保存")
        ...
        # 交易逻辑
        st.subheader("📝 交易逻辑")
        new_logic = st.text_area("买卖逻辑描述（支持多行）", value=logic_text, height=100, key="logic_input")
        if st.button("保存交易逻辑"):
            c.execute("INSERT OR REPLACE INTO stock_logic (code, logic) VALUES (?, ?)", (selected_stock, new_logic))
            conn.commit()
            thread = threading.Thread(target=sync_db_to_github, daemon=True)
            thread.start()
            st.success("逻辑已保存")
        ...
        # 决策历史
        st.subheader("📅 决策历史")
        decisions_df = pd.read_sql(f"SELECT id, date, decision, reason FROM decisions WHERE code = '{selected_stock}' ORDER BY date DESC", conn)
        if not decisions_df.empty:
            for _, row in decisions_df.iterrows():
                col_d1, col_d2, col_d3 = st.columns([2, 4, 1])
                with col_d1:
                    st.write(f"**{row['date']}**")
                with col_d2:
                    st.write(f"**决策：** {row['decision']}")
                    if row['reason']:
                        st.write(f"*理由：* {row['reason']}")
                with col_d3:
                    if st.button("🗑️", key=f"del_decision_{row['id']}"):
                        c.execute("DELETE FROM decisions WHERE id = ?", (row['id'],))
                        conn.commit()
                        thread = threading.Thread(target=sync_db_to_github, daemon=True)
                        thread.start()
                        st.rerun()
        ...
        with st.expander("➕ 添加新决策"):
            col_d1, col_d2 = st.columns(2)
            new_decision_date = col_d1.date_input("决策日期", datetime.now())
            new_decision = col_d2.text_input("决策内容")
            new_reason = st.text_area("决策理由", height=80)
            if st.button("保存决策"):
                if new_decision.strip():
                    c.execute("INSERT INTO decisions (code, date, decision, reason) VALUES (?, ?, ?, ?)",
                              (selected_stock, new_decision_date.strftime('%Y-%m-%d'), new_decision.strip(), new_reason.strip() if new_reason.strip() else None))
                    conn.commit()
                    thread = threading.Thread(target=sync_db_to_github, daemon=True)
                    thread.start()
                    st.success("决策已添加")
        ...
        # 涨跌周期
        st.subheader("📈 涨跌周期记录")
        cycles_df = pd.read_sql(f"SELECT id, start_date, end_date, change_percent, direction FROM cycles WHERE code = '{selected_stock}' ORDER BY start_date", conn)
        
        # 计算平均涨幅/跌幅
        if not cycles_df.empty:
            up_cycles = cycles_df[cycles_df['direction'] == 'up']['change_percent']
            down_cycles = cycles_df[cycles_df['direction'] == 'down']['change_percent']
            avg_up = up_cycles.mean() if not up_cycles.empty else 0.0
            avg_down = down_cycles.mean() if not down_cycles.empty else 0.0
            st.write(f"📊 **平均涨幅：** {avg_up:.2f}%  **平均跌幅：** {avg_down:.2f}%")
        else:
            st.info("暂无周期记录")

        # 显示已有周期
        if not cycles_df.empty:
            html = '<table class="custom-table"><thead><tr><th>开始日期</th><th>结束日期</th><th>涨跌幅</th><th>方向</th><th>操作</th></tr></thead><tbody>'
            for _, row in cycles_df.iterrows():
                color = "profit-red" if row['direction'] == 'up' else "loss-green"
                html += f"<tr><td>{row['start_date']}</td><td>{row['end_date']}</td><td class='{color}'>{row['change_percent']:+.2f}%</td><td>{'上涨' if row['direction']=='up' else '下跌'}</td>"
                html += f"<td><button key='del_cycle_{row['id']}' onclick='alert(\"删除需通过下方按钮\")' style='background:transparent;border:none;color:#888;cursor:default;'>🗑️</button></td></tr>"
            html += '</tbody></table>'
            st.markdown(html, unsafe_allow_html=True)
            # 实际删除按钮单独放在下方
            del_id = st.selectbox("选择要删除的周期ID", options=[0] + cycles_df['id'].tolist(), format_func=lambda x: f"周期 {x}" if x else "无")
            if del_id and st.button("删除选中周期"):
                c.execute("DELETE FROM cycles WHERE id = ?", (del_id,))
                conn.commit()
                thread = threading.Thread(target=sync_db_to_github, daemon=True)
                thread.start()
                st.success("已删除")
        ...
        # 添加周期
        with st.expander("➕ 添加新周期"):
            col_c1, col_c2, col_c3 = st.columns(3)
            start_d = col_c1.date_input("开始日期", datetime.now())
            end_d = col_c2.date_input("结束日期", datetime.now())
            change = col_c3.number_input("涨跌幅 (%)", value=0.0, step=0.1, format="%.2f")
            direction = "up" if change >= 0 else "down"
            if st.button("保存周期"):
                if start_d <= end_d:
                    c.execute("INSERT INTO cycles (code, start_date, end_date, change_percent, direction) VALUES (?, ?, ?, ?, ?)",
                              (selected_stock, start_d.strftime('%Y-%m-%d'), end_d.strftime('%Y-%m-%d'), change, direction))
                    conn.commit()
                    thread = threading.Thread(target=sync_db_to_github, daemon=True)
                    thread.start()
                    st.success("周期已添加")
        ...
        # 历史平均年化收益率（手动录入或简单估算）
        st.subheader("📊 历史平均年化收益率")
        # 这里可以放一个手动输入框，或者从交易记录简单估算。为简化，让用户手动录入。
        # 我们可以在 stock_peak 表中增加一列 annual_return，或者单独建表。这里简单用 session_state 暂存，但为了持久化，可以存在 stock_logic 表里，增加字段。
        # 简单做法：在 stock_logic 表中增加 annual_return 字段，或者另建表 stock_stats。为减少复杂度，我们直接在 stock_logic 表里加一列 annual_return。
        # 先检查列是否存在，不存在则添加
        try:
            c.execute("ALTER TABLE stock_logic ADD COLUMN annual_return REAL DEFAULT 0.0")
            conn.commit()
        except sqlite3.OperationalError:
            pass
        annual_row = c.execute("SELECT annual_return FROM stock_logic WHERE code = ?", (selected_stock,)).fetchone()
        annual_return = annual_row[0] if annual_row else 0.0
        new_annual = st.number_input("年化收益率 (%)", value=annual_return, step=0.1, format="%.2f")
        if st.button("保存年化收益率"):
            c.execute("UPDATE stock_logic SET annual_return = ? WHERE code = ?", (new_annual, selected_stock))
            if c.rowcount == 0:
                c.execute("INSERT INTO stock_logic (code, annual_return) VALUES (?, ?)", (selected_stock, new_annual))
            conn.commit()
            thread = threading.Thread(target=sync_db_to_github, daemon=True)
            thread.start()
            st.success("已保存")
